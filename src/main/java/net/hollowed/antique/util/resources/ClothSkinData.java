...
public record ClothSkinData(
    ...
    public static final Codec<ClothSkinData> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ClothSubData.CODEC.listOf().fieldOf("skins").orElseGet(() -> List.of(new ClothSubData("", "", 0, 0, 0, 0, false))).forGetter(ClothSkinData::list)
    ).apply(instance, ClothSkinData::new));

    public record ClothSubData(String model, String hex, float length, float width, int bodyAmount, int light, boolean overlay) {
